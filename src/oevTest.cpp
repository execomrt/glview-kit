...
; *
; * 	Copyright (C) Realtech VR 2000 - 2021 - http://www.realtech-vr.com/glview
; *
...
/// <returns></returns>
int run_rendering_test(ovRenderer renderer, int fullscreen, int width, int height, int vsync, int fog, int transparency, int user_clip_pane, string multisampling, string anisotropy, string texture_load, string pixel_format, string scene, string fbformat, int test_duration)
{
	scene = "0";
	std::string szXml;
	int option = 0;
	if (fullscreen) {
		option |= (WGLDIAG_OPTION_FS);
	}
	else {
	...
	if (fbformat == "FLOAT") {
		option |= (WGLDIAG_OPTION_FB_FLOAT32); // Enable HDR
	}
	...
	szXml = szXml + CreateElement("anisotropy", anisotropy);
	...
	else if (renderer == RENDERER_VK1)
	{
		szXml = szXml + CreateElement("test", "1.1");
	}
	szXml = szXml + CreateElement("fbenable", "Simple");
	...
	/*
	szXml = "<root><option>1061920</option><duration>20</duration><multisample>8</multisample><anisotropy>0</anisotropy><texturelod>0</texturelod><displaymode>48</displaymode><renderer>12</renderer><pixelformat>1</pixelformat><test>4.2;4.3;4.4;4.5</tet><fbenable>Simple</fbenable><fbformat>RGB</fbformat><scene>0</scene><width>1920</width><height>1080</height></root>";
	szXml = "<root><test>3.0;3.1;3.2;3.3;4.0;4.1;4.2;4.3;4.4;4.5</test><option>0</option><anisotropy>0</anisotropy><multisample>0</multisample><texturelod>0</texturelod><renderer>12</renderer><pixelformat>1</pixelformat><fbenable>Simple</fbenable><duration>6</duration><scene>0</scene><fbformat>RGB</fbformat><displaymode>72</displaymode></root>";
	szXml = "<root><test>4.2;4.3;4.4;4.5</test><option>0</option><anisotropy>0</anisotropy><multisample>8</multisample><texturelod>0</texturelod><renderer>12</renderer><pixelformat>1</pixelformat><fbenable>Simple</fbenable><duration>6</duration><scene>0</scene><fbformat>RGB</fbformat><displaymode>72</displaymode><width>1920</width><height>1080</height></root>";
	*/
	// BUG
	szXml = "<root><test>3.0;3.1;3.2;3.3;4.0;4.1;4.2;4.3;4.4;4.5</test><option>1048576</option><anisotropy>0</anisotropy><multisample>0</multisample><texturelod>0</texturelod><renderer>12</renderer><pixelformat>1</pixelformat><fbenable>Simple</fbenable><duration>6</duration><scene>0</scene><fbformat>RGB</fbformat><displaymode>72</displaymode><width>1920</width><height>1080</height></root>";

	// BUG
//	szXml = "<root><test>3.0;3.1;3.2;3.3;4.0;4.1;4.2;4.3;4.4;4.5</test><option>1061920</option><anisotropy>0</anisotropy><multisample>0</multisample><texturelod>0</texturelod><renderer>12</renderer><pixelformat>1</pixelformat><fbenable>Simple</fbenable><duration>6</duration><scene>0</scene><fbformat>RGB</fbformat><displaymode>72</displaymode><width>1920</width><height>1080</height></root>";
...
int PASCAL WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (funcInitWad("GLVIEW.RMX") >= 0)
	{
		...
		*/
		...
		// auto renderer = RENDERER_VK1;
		return run_rendering_test(renderer, 0, 1920, 1080, 1, 1, 1, 1, "8", "0", "0", "1", "3", "RGB", 20);
	}
	else
	{
		return -1;
